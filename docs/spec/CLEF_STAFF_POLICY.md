...
この文書は、`miku-score` における MusicXML 出力時の譜表/記号判定ルールを定義する。

- 主軸: VSQX->MusicXML の現行ルール（55/64 閾値・ヒステリシス）を基準にする。
- 例外: ドラム系の扱いは `miku-score` 現行実装を基準にする。
...
`miku-score` 現行には「空レーンでも最低1 voice を確保する」出力安定化フォールバックがある。
